...
//Animate bottom nav on section scrolling
//Out
const animate_out_section_scroll_nav = () => {
  $('#section_nav_container_down').slideUp(200, function(){
    $('#prev_section_icon').hide();
    $('#next_section_icon').hide();
  });
}

//In
const animate_in_section_scroll_nav = (prio = 1, time = 200) => {
  if(scroll_lock.enter(prio, true)){
    if(global.isNotCurrentSectionFirst()){
      $('#prev_section_icon').show();
    }
    if(global.isNotCurrentSectionLast()){
      $('#next_section_icon').show();
    }
    $('#section_nav_container_down').slideDown(time, function(){
      scroll_lock.leave(prio);
    });
  }
}

//Section scrolling functions
const section_scroll_to = (section = global.currentSection, time = 0, nav_anim = true) => {
  if(nav_anim){animate_out_section_scroll_nav();}
  $('html, body').animate({scrollTop: global.get_sections(section)}, time, function() {
    if(nav_anim){animate_in_section_scroll_nav()};
    global.fitCurrentY();
    scroll_lock.leave();
  });
}

/*const scroll_top = (section = 0, time = 0, nav_anim = true) => {
  if(nav_anim){animate_out_section_scroll_nav();}
  $('html, body').animate({scrollTop: section}, time, $.bez([.34,.15,.25,1]), function() {
    if(nav_anim){animate_in_section_scroll_nav()};
    scroll_lock.leave();
  });
}*/
...
//Wheel handler
const section_scroll_handler_wheel = (deltaY) => {
  if(scroll_lock.enter()){
    if(deltaY > 0){
      section_scroll_down(1000);
    }else if(deltaY < 0){
      section_scroll_up(1000);
    }
  }
}

const section_scroll_down = (time = 1000) => {
  if(global.isNotCurrentSectionLast()){
    ++global.currentSection;
    section_scroll_to(global.currentSection, time);
  }else{
...
const section_scroll_up = (time = 1000) => {
  if(global.isNotCurrentSectionFirst()){
    --global.currentSection;
    section_scroll_to(global.currentSection, time);
  }else{
...
//Bottom nav handlers
const section_scroll_handler_next_button = () => {
  if(scroll_lock.enter()){
    section_scroll_down(1000);
  }
}

const section_scroll_handler_prev_button = () => {
  if(scroll_lock.enter()){
    section_scroll_up(1000);
  }
}
